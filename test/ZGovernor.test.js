...
contract('Governor', ([alice, minter, dev]) => {
    it('should work', async () => {
        ...
        await this.taco.delegate(dev, { from: dev });
        this.chef = await MasterChef.new(this.taco.address, dev, '100', '0', '0', { from: alice });
        ...
        // Perform another deposit to make sure some TACOs are minted in that 1 block.
        await this.chef.deposit(0, '100', { from: minter });
        assert.equal((await this.taco.totalSupply()).valueOf(), '110');
        assert.equal((await this.taco.balanceOf(minter)).valueOf(), '100');
        assert.equal((await this.taco.balanceOf(dev)).valueOf(), '10');
        // Transfer ownership to timelock contract
        this.timelock = await Timelock.new(alice, time.duration.days(2), { from: alice });
        this.gov = await GovernorAlpha.new(this.timelock.address, this.taco.address, alice, { from: alice });
