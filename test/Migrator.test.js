...
const TacoSwapPair = artifacts.require('TacoSwapPair');
const TacoSwapFactory = artifacts.require('TacoSwapFactory');
const Migrator = artifacts.require('Migrator');

contract('Migrator', ([alice, bob, dev, minter]) => {
    beforeEach(async () => {
        this.factory1 = await TacoSwapFactory.new(alice, { from: alice });
        this.factory2 = await TacoSwapFactory.new(alice, { from: alice });
        ...
        this.lp1 = await TacoSwapPair.at((await this.factory1.createPair(this.weth.address, this.token.address)).logs[0].args.pair);
        this.lp2 = await TacoSwapPair.at((await this.factory2.createPair(this.weth.address, this.token.address)).logs[0].args.pair);
        this.chef = await MasterChef.new(this.taco.address, dev, '1000', '0', '100000', { from: alice });
    ...
    it('should allow first minting from public only after migrator is gone', async () => {
        ...
        this.lpx = await TacoSwapPair.at((await this.factory2.createPair(this.weth.address, this.tokenx.address)).logs[0].args.pair);
